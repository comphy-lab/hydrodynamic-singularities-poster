...
.p-pivot {
  ...
  margin: 3mm auto 7mm; padding: 8mm 22mm;
  border-radius: var(--r-sm);
  background: color-mix(in srgb, var(--c-accent-coral) 8%, var(--c-surface-strong));
  border: 2px solid color-mix(in srgb, var(--c-accent-coral) 32%, transparent);
  font-family: var(--t-serif);
  font-size: 39pt; line-height: 1.05; color: var(--fg-strong);
}
.p-pivot::before {
  content: "Q"; flex: 0 0 auto;
  display: grid; place-items: center;
  width: 1.35em; height: 1.35em;
  background: var(--c-accent-coral); color: var(--c-surface-strong);
  border-radius: var(--r-xs);
  font-weight: var(--t-weight-bold); font-size: 0.82em; line-height: 1;
}
.p-pivot em { font-style: normal; color: var(--c-accent-coral); font-weight: var(--t-weight-semi); }
